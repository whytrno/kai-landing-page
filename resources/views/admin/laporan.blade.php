@extends('includes.main')

@section('content')
    <div class="bg-[url('{{ asset('images/bg2.png') }}')] bg-cover">
        <div class="py-5 px-10 space-y-10">
            <div class="flex justify-between">
                <h1 class="font-bold text-2xl">User Setting</h1>

                <div id="print-button"
                    class="flex items-center gap-2 rounded-lg bg-blue-700 stroke-white text-white py-2 px-5 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0021 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 00-1.913-.247M6.34 18H5.25A2.25 2.25 0 013 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 011.913-.247m10.5 0a48.536 48.536 0 00-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5zm-3 0h.008v.008H15V10.5z" />
                    </svg>

                    <p>Print</p>
                </div>
            </div>
            <div class="space-y-5 bg-white rounded-t-lg">
                <div class="flex gap-2 justify-center bg-[#277ACD] py-2 text-white font-semibold rounded-lg">
                    <p>Laporan</p>
                </div>

                <div class="px-10 pb-10 space-y-5">
                    <div>
                        <table class="w-full text-[#152A75]">
                            <tr>
                                <th class="border border-[#277ACD]">No</th>
                                <th class="border border-[#277ACD]">Nama Pengguna</th>
                                <th class="border border-[#277ACD]">Judul Video</th>
                                <th class="border border-[#277ACD]">Ditonton Pada</th>
                            </tr>
                            @foreach ($data as $d)
                                <tr class="text-center">
                                    <td class="border border-[#277ACD]">{{ $loop->iteration }}</td>
                                    <td class="border border-[#277ACD]">{{ $d->user->nama }}</td>
                                    <td class="border border-[#277ACD]">{{ $d->video->judul }}</td>
                                    <td class="border border-[#277ACD]">{{ $d->created_at->format('d F Y') }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@push('scripts')
    <script>
        $('#print-button').click(function() {
            window.print();
        });
    </script>
@endpush
